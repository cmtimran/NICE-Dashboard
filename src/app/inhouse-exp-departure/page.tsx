...
export default function InhouseExpDeparture() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch(`/api/inhouse-exp-departure?startDate=${startDate}&endDate=${endDate}`);
    ...
    const handlePrint = useReactToPrint({
        ...
        documentTitle: `Inhouse_Exp_Dep_${startDate}`,
    });

    const handleExport = (type: 'csv' | 'excel' | 'pdf' | 'copy') => {
        const headers = ['Room', 'Guest', 'Company', 'Nationality', 'Arrival', 'Departure', 'Status'];
        ...
        if (type === 'copy') {
            ...
            link.setAttribute('download', `inhouse_exp_departure_${startDate}.csv`);
            ...
            XLSX.utils.book_append_sheet(wb, ws, "Inhouse_Exp_Dep");
            XLSX.writeFile(wb, "inhouse_exp_departure.xlsx");
        } else if (type === 'pdf') {
            ...
            doc.text("Inhouse Expected Departure", 14, 15);
            doc.save("inhouse_exp_departure.pdf");
        }
    };
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 p-6">
            <div>
                <h1 className="text-3xl font-bold text-white">Inhouse Exp. Departure</h1>
                <p className="text-gray-400">Departures expected from current in-house guests.</p>
            </div>
                            ...
                            {loading ? (
                                ...
                                paginatedData.map((row, i) => (
                                    <tr key={i} className="hover:bg-white/5 transition-colors print:hover:bg-transparent">
                                        ...
                                        <td className="px-6 py-4 text-gray-400 print:text-black">{row.fldStatus}</td>
                                    </tr>
                                ))
                            )}
                        </tbody>
