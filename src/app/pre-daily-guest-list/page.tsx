...
export default function PreDailyGuestList() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch(`/api/pre-daily-guest-list?startDate=${startDate}&endDate=${endDate}`);
    ...
    const handlePrint = useReactToPrint({
        ...
        documentTitle: `Pre_Daily_Guest_${startDate}`,
    });
    ...
    const handleExport = (type: 'csv' | 'excel' | 'pdf' | 'copy') => {
        ...
        if (type === 'copy') {
            ...
            link.setAttribute('download', `pre_daily_guest_${startDate}.csv`);
            ...
            XLSX.utils.book_append_sheet(wb, ws, "Pre_Daily_Guest");
            XLSX.writeFile(wb, "pre_daily_guest_list.xlsx");
        } else if (type === 'pdf') {
            ...
            doc.text("Pre. Daily Guest List", 14, 15);
            doc.save("pre_daily_guest_list.pdf");
        }
    };
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 p-6">
            <div>
                <h1 className="text-3xl font-bold text-white">Pre. Daily Guest List</h1>
                <p className="text-gray-400">Historical guest list records.</p>
            </div>
