...
export default function ExpectedArrivalPage() {
    ...
    const handlePrint = useReactToPrint({
        ...
        documentTitle: `Expected Arrivals ${startDate} to ${endDate}`,
    });
    ...
    const handleExport = (type: 'csv' | 'excel' | 'pdf' | 'copy') => {
        ...
        const filename = `Expected_Arrivals_${startDate}_to_${endDate}`;
        ...
        switch (type) {
            ...
            case 'pdf':
                ...
                exportToPDF(headers, exportData, `Expected Arrivals (${startDate} to ${endDate})`, filename);
                break;
    ...
    const fetchData = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`/api/expected-arrival?startDate=${startDate}&endDate=${endDate}`);
    ...
    return (
        <div className="space-y-8">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-white tracking-tight">Expected Arrival</h1>
                    <p className="text-gray-400 mt-1">Upcoming guest arrivals and reservation details</p>
                </div>
                        ...
                        <tbody className="divide-y divide-white/5 text-gray-300 print:divide-gray-200">
                            {loading ? (
                                ...
                                paginatedData.map((row, idx) => (
                                    <tr key={idx} className="hover:bg-white/5 transition-colors group print:hover:bg-transparent">
                                        <td className="p-4 font-mono text-blue-400 print:text-black">{row.reservationNo}</td>
                                        <td className="p-4 font-bold text-white print:text-black">{row.guestName}</td>
                                        <td className="p-4 print:text-black">{row.companyName}</td>
                                        <td className="p-4 print:text-black">
                                            <div className="text-sm">{row.roomType}</div>
                                            <div className="text-xs text-gray-500 print:text-gray-700">Room: {row.roomNo}</div>
                                        </td>
                                        <td className="p-4 print:text-black">{row.arrivalDate}</td>
                                        <td className="p-4 print:text-black">{row.departureDate}</td>
                                        <td className="p-4">
                                            <span className="px-2 py-1 rounded-lg text-xs bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 print:bg-transparent print:border-none print:text-black print:p-0">
                                                {row.status}
                                            </span>
                                        </td>
                                    </tr>
