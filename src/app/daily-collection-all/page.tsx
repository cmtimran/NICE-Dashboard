...
export default function DailyCollectionAll() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch(`/api/daily-collection-all?startDate=${startDate}&endDate=${endDate}`);
    ...
    const handlePrint = useReactToPrint({
        ...
        documentTitle: `Daily_Collection_All_${startDate}`,
    });
    ...
    const handleExport = (type: 'csv' | 'excel' | 'pdf' | 'copy') => {
        ...
        if (type === 'copy') {
            ...
            link.setAttribute('download', `daily_collection_all_${startDate}.csv`);
            ...
            XLSX.utils.book_append_sheet(wb, ws, "Daily_Collection_All");
            XLSX.writeFile(wb, "daily_collection_all.xlsx");
        } else if (type === 'pdf') {
            ...
            doc.text("Daily Collection All Report", 14, 15);
            doc.save("daily_collection_all.pdf");
        }
    };
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 p-6">
            <div className="flex flex-col gap-6">
                <div>
                    <h1 className="text-3xl font-bold text-white">Daily Collection (All)</h1>
                    <p className="text-gray-400">Complete breakdown of daily collections.</p>
                </div>
