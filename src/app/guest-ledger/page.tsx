...
import { Download, Printer, Search, FileText, Copy, FileSpreadsheet, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function GuestLedger() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch('/api/guest-ledger');
    ...
    const handleExport = (type: 'csv' | 'excel' | 'pdf' | 'copy') => {
        ...
        if (type === 'copy') {
            ...
            link.setAttribute('download', 'guest_ledger.csv');
            ...
            XLSX.utils.book_append_sheet(wb, ws, "Guest_Ledger");
            XLSX.writeFile(wb, "guest_ledger.xlsx");
        } else if (type === 'pdf') {
            ...
            doc.text("Guest Ledger", 14, 15);
            doc.save("guest_ledger.pdf");
        }
    };
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 p-6">
            <div className="flex flex-col gap-6">
                <div>
                    <h1 className="text-3xl font-bold text-white">Guest Ledger</h1>
                    <p className="text-gray-400">Current guest balances.</p>
                </div>
