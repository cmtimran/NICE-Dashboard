...
export default function ReservationExpDeparture() {
    const componentRef = useRef<HTMLDivElement>(null);
    const [data, setData] = useState<ResExpDeparture[]>([]);
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch(`/api/reservation-exp-departure?startDate=${startDate}&endDate=${endDate}`);
    ...
    const handlePrint = useReactToPrint({
        ...
        documentTitle: `Res_Exp_Departure_${startDate}`,
    });

    const handleExport = (type: 'csv' | 'excel' | 'pdf' | 'copy') => {
        const headers = ['Booking No', 'Guest Name', 'Company', 'Room Type', 'Arrival', 'Departure', 'Phone', 'Status'];
        ...
        if (type === 'copy') {
            ...
            link.setAttribute('download', `res_exp_departure_${startDate}.csv`);
            ...
            XLSX.utils.book_append_sheet(wb, ws, "Exp_Departure");
            XLSX.writeFile(wb, "res_exp_departure.xlsx");
        } else if (type === 'pdf') {
            ...
            doc.text("Reservation Expected Departure", 14, 15);
            doc.save("res_exp_departure.pdf");
        }
    };
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 p-6">
            <div>
                <h1 className="text-3xl font-bold text-white">Reservation Exp. Departure</h1>
                <p className="text-gray-400">Expected departures based on reservations.</p>
            </div>
                            ...
                            {loading ? <tr><td colSpan={8} className="px-6 py-12 text-center text-gray-500">Loading...</td></tr> :
                                paginatedData.map((item, i) => (
                                    <tr key={i} className="hover:bg-white/5 transition-colors print:hover:bg-transparent">
                                        ...
                                        <td className="px-6 py-4 text-gray-400 print:text-black">{item.fldStatus}</td>
                                    </tr>
                                ))}
                        </tbody>
                    </table>
