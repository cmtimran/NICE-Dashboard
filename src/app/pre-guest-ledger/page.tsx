...
export default function PreGuestLedger() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch(`/api/pre-guest-ledger?date=${date}`);
    ...
    const handlePrint = useReactToPrint({
        ...
        documentTitle: `Pre_Guest_Ledger_${date}`,
    });

    const handleExport = (type: 'csv' | 'excel' | 'pdf' | 'copy') => {
        const headers = ['Room', 'Guest Name', 'Company', 'Arrival', 'Departure', 'Rate', 'Total Charges', 'Total Credits', 'Balance'];
        const rows = data.map(item => [
            item.roomno,
            item.fullname,
            item.nameco,
            new Date(item.doar).toLocaleDateString(),
            new Date(item.dod).toLocaleDateString(),
            item.Rate,
            item.total,
            item.credit,
            item.balance
        ...
        if (type === 'copy') {
            ...
            link.setAttribute('download', `pre_guest_ledger_${date}.csv`);
            ...
            XLSX.utils.book_append_sheet(wb, ws, "Pre_Guest_Ledger");
            XLSX.writeFile(wb, "pre_guest_ledger.xlsx");
        } else if (type === 'pdf') {
            ...
            doc.text("Pre. Guest Ledger", 14, 15);
            doc.save("pre_guest_ledger.pdf");
        }
    };
    ...
    const filteredData = data.filter(item =>
        ...
        item.nameco?.toLowerCase().includes(searchTerm.toLowerCase())
    );

    const totalPages = Math.ceil(filteredData.length / itemsPerPage);
    const paginatedData = filteredData.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);

    useEffect(() => { setCurrentPage(1); }, [searchTerm, date]);

    return (
        <div className="space-y-8 animate-in fade-in duration-500 p-6">
            <div className="flex flex-col gap-6">
                <div>
                    <h1 className="text-3xl font-bold text-white">Pre. Guest Ledger</h1>
                    <p className="text-gray-400">Historical guest ledger snapshot.</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 bg-white/5 p-2 rounded-2xl border border-white/10 w-full print:hidden">
                    ...
                    <div className="relative flex-1 min-w-[200px]">
                        ...
                        <input
                            type="text"
                            placeholder="Search Guest, Room, Company..."
                            value={searchTerm}
                    ...
                    <div className="flex items-center gap-2 bg-black/20 p-1.5 rounded-xl border border-white/10">
                        ...
                        <input type="date" value={date} onChange={e => setDate(e.target.value)} className="bg-transparent text-white text-sm outline-none w-[110px]" />
                            ...
                            {loading ? (
                                ...
                                paginatedData.map((row, i) => (
                                    <tr key={i} className="hover:bg-white/5 transition-colors print:hover:bg-transparent">
                                        <td className="px-6 py-4 font-mono text-blue-300 print:text-black">{row.roomno}</td>
                                        <td className="px-6 py-4 text-white print:text-black">{row.fullname}</td>
                                        <td className="px-6 py-4 text-gray-400 print:text-black">{row.nameco}</td>
                                        <td className="px-6 py-4 text-gray-400 print:text-black">{new Date(row.doar).toLocaleDateString()}</td>
                                        <td className="px-6 py-4 text-gray-400 print:text-black">{new Date(row.dod).toLocaleDateString()}</td>
                                        <td className="px-6 py-4 text-gray-300 print:text-black">{row.Rate}</td>
                                        <td className="px-6 py-4 text-gray-300 print:text-black">{row.total}</td>
                                        <td className="px-6 py-4 text-green-400 print:text-black">{row.credit}</td>
                                        <td className="px-6 py-4 font-bold text-white print:text-black">{row.balance}</td>
                                    </tr>
                                ))
                            )}
                        </tbody>
