...
import { Search, FileText, Printer, Copy, FileSpreadsheet, File, ChevronLeft, ChevronRight } from 'lucide-react';
// @ts-ignore
import { useReactToPrint } from 'react-to-print';
import { exportToCSV, exportToExcel, exportToPDF, copyToClipboard } from '@/lib/ExportUtils';

export default function DailyGuestListPage() {
    ...
    const handlePrint = useReactToPrint({
        ...
        documentTitle: `Daily Guest List ${new Date().toISOString().split('T')[0]}`,
    });

    useEffect(() => {
        fetch('/api/daily-guest-list')
            .then(res => res.json())
            .then(d => {
                if (!d.error) {
                    setData(d);
    ...
    useEffect(() => {
        ...
        const filtered = data.filter(item =>
            ...
            item.roomNo?.toString().includes(lower) ||
            item.regNo?.toString().includes(lower) ||
    ...
    const handleExport = (type: 'csv' | 'excel' | 'pdf' | 'copy') => {
        ...
        const exportData = filteredData.map((row, i) => ({
            ...
            'Exp. Out': row.checkOutDate,
            'Time': row.checkOutTime,
            'Status': row.status || 'Active'
        }));

        const dateStr = new Date().toISOString().split('T')[0];
        const filename = `Daily_Guest_List_${dateStr}`;
        ...
        switch (type) {
            ...
            case 'pdf':
                ...
                exportToPDF(headers, exportData, `Daily Guest List (${dateStr})`, filename);
                break;
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col gap-6">
                <div>
                    <h1 className="text-3xl font-bold text-white">Daily Guest List</h1>
                    <p className="text-gray-400">Current in-house guest details</p>
                </div>

                <div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
                    {/* Export Buttons */}
                    <div className="flex flex-wrap gap-2 print:hidden bg-white/5 p-1.5 rounded-xl border border-white/10 w-full lg:w-auto">
                    ...
                    <table className="w-full text-sm text-left whitespace-nowrap">
                        <thead className="bg-white/5 text-gray-300 font-semibold border-b border-white/10 print:bg-gray-100 print:text-black">
                            <tr>
                                ...
                                <th className="px-6 py-4">Rate (TK)</th>
                                <th className="px-6 py-4">Check In</th>
                                <th className="px-6 py-4">Exp. Out</th>
                        ...
                        <tbody className="divide-y divide-white/5 print:divide-gray-200">
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={10} className="px-6 py-8 text-center text-gray-500">No guests found</td>
                                </tr>
                                ...
                                <tr key={i} className="hover:bg-white/5 transition-colors group print:hover:bg-transparent">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className="bg-blue-500/20 text-blue-300 px-2 py-1 rounded-md font-mono font-bold group-hover:bg-blue-500/30 transition-colors print:bg-transparent print:text-black print:p-0">
                                            {row.roomNo}
                                        </span>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 text-gray-400 print:text-black">{row.checkInDate}</td>
                                    <td className="px-6 py-4 text-gray-400 print:text-black">{row.checkOutDate}</td>
                                    <td className="px-6 py-4 text-gray-500 text-xs print:text-black">{row.checkOutTime}</td>
                                    <td className="px-6 py-4">
                                        <span className="px-2 py-1 rounded-full bg-green-500/10 text-green-400 text-xs border border-green-500/20 print:bg-transparent print:border-none print:text-black print:p-0">
                                            {row.status || 'Active'}
                                        </span>
                                    </td>
                                </tr>
