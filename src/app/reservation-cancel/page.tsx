...
export default function ReservationCancel() {
    ...
    const [data, setData] = useState<CancelReservation[]>([]);
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch(`/api/reservation-cancel?startDate=${startDate}&endDate=${endDate}`);
    ...
    const handlePrint = useReactToPrint({
        ...
        documentTitle: `Cancelled_Res_${startDate}`,
    });

    const handleExport = (type: 'csv' | 'excel' | 'pdf' | 'copy') => {
        const headers = ['Cancel Date', 'Booking No', 'Guest Name', 'Company', 'Room Type', 'Arrival', 'Departure', 'Reason', 'User'];
        const rows = data.map(item => [
            new Date(item.CanDate).toLocaleDateString(),
            item.reservation_no,
        ...
        if (type === 'copy') {
            ...
            link.setAttribute('download', `cancelled_res_${startDate}.csv`);
            ...
            XLSX.utils.book_append_sheet(wb, ws, "Cancelled_Res");
            XLSX.writeFile(wb, "cancelled_reservations.xlsx");
        } else if (type === 'pdf') {
            ...
            doc.text("Cancelled Reservations", 14, 15);
            doc.save("cancelled_reservations.pdf");
        }
    };
    ...
    // ... (fetchData and handleExport remain same)
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 p-6">
            <div>
                <h1 className="text-3xl font-bold text-white">Reservation Cancel</h1>
                <p className="text-gray-400">Report of cancelled reservations.</p>
            </div>

            {/* Toolbar: Exports, Search, Date Filter in one row */}
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left whitespace-nowrap">
                        <thead className="bg-white/5 text-gray-300 font-semibold border-b border-white/10 print:bg-gray-100 print:text-black">
                            <tr>
                                <th className="px-6 py-4">Cancel Date</th>
                            ...
                            {loading ? (
                                ...
                                paginatedData.map((item, index) => (
                                    <tr key={index} className="hover:bg-white/5 transition-colors print:hover:bg-transparent">
                                        <td className="px-6 py-4 text-gray-400 print:text-black">{new Date(item.CanDate).toLocaleDateString()}</td>
                                        ...
                                        <td className="px-6 py-4 text-red-400/80 italic print:text-black">{item.CancelReason}</td>
                                        <td className="px-6 py-4 text-gray-500 print:text-black">{item.CancelUser}</td>
                                    </tr>
                                ))
                            )}
                        </tbody>
