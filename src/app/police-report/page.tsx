...
export default function PoliceReport() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch('/api/police-report');
    ...
    const handleExport = (type: 'csv' | 'excel' | 'pdf' | 'copy') => {
        const headers = ['Room', 'Guest Name', 'Nationality', 'Passport', 'Arrival', 'Departure', 'Address'];
        ...
        if (type === 'copy') {
            ...
            XLSX.utils.book_append_sheet(wb, ws, "Police_Report");
            XLSX.writeFile(wb, "police_report.xlsx");
        } else if (type === 'pdf') {
            ...
            doc.text("Police Report (Foreigner)", 14, 15);
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 p-6">
            <div className="flex flex-col gap-6">
                <div>
                    <h1 className="text-3xl font-bold text-white">Police Report (Foreigner)</h1>
                    <p className="text-gray-400">Current in-house foreign guests.</p>
                </div>
